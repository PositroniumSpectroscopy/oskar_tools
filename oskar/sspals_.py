            ...
            if ntraces > 0:
                ...
                for col in columns:
                    if col in sspalsDF:
                        ...
                        avDF.loc[sq, col+'_ra_reps'] = rep
                        avDF.loc[sq, col+'_ra_mean'] = sspalsDF[col].mean()
                        avDF.loc[sq, col+'_ra_std'] = sspalsDF[col].std()
                        avDF.loc[sq, col+'_ra_sem'] = sspalsDF[col].std()/np.sqrt(rep)
    if verbose:
...
def sspals_loop_h5(data, hi, low, vDF, uDF, **kwargs):
    ...
    for vid in tqdm(uDF.index, smoothing=0.1, unit=' vids',
        ...
        if ntraces > 0:
            ...
            for col in columns:
                if col in sspalsDF:
                    rep = sspalsDF[col].count()
                    avDF.loc[vid, col+'_ra_reps'] = rep
                    avDF.loc[vid, col+'_ra_mean'] = sspalsDF[col].mean()
                    avDF.loc[vid, col+'_ra_std'] = sspalsDF[col].std()
                    avDF.loc[vid, col+'_ra_sem'] = sspalsDF[col].std()/np.sqrt(rep)
    if verbose:
        print('remove empty columns')
    avDF = avDF.dropna(axis=1, how='all')
    if len(avDF.columns) > 0:
...
def main():
    ...
    for rid in args.rid:
        ...
        with h5py.File(h5.fil, 'r') as dfil:
            ...
            sys.stdout.flush()
            if args.loop:
                avDF = sspals_loop_h5(data, hi, low, vDF, uDF, **vars(args))
            else:
                avDF = sspals_h5(data, hi, low, **vars(args))
            if args.verbose:
