...
def average_h5(data, name, columns, **kwargs):
    ...
    for sq in tqdm(squids, smoothing=0.1, unit=' squids', leave=True,
        ...
        if name in data[squid]:
            raw_df = pd.DataFrame(np.array(data[squid][name]))
            for col in columns:
                if col in raw_df.columns:
                    rep = raw_df[col].count()
                    av_df.loc[sq, col+'_reps'] = rep
                    av_df.loc[sq, col+'_mean'] = raw_df[col].mean()
                    av_df.loc[sq, col+'_std'] = raw_df[col].std()
                    av_df.loc[sq, col+'_sem'] = raw_df[col].std()/np.sqrt(rep)
    if verbose:
...
def average_loop_h5(data, name, columns, v_df, u_df, **kwargs):
    ...
    if len(columns) > 0:
        ...
        for vid in tqdm(u_df.index, smoothing=0.1, unit=' vids',
            ...
            if max_squid > 0:
                squids = squids[squids <= max_squid]
            if len(squids) > 0:
                raw_df = oskar.h5_df(data, squids, name, ignore_missing=True)
                if len(raw_df) > 0:
                    for col in columns:
                        if col in raw_df.columns:
                            rep = raw_df[col].count()
                            av_df.loc[vid, col+'_reps'] = rep
                            av_df.loc[vid, col+'_mean'] = raw_df[col].mean()
                            av_df.loc[vid, col+'_std'] = raw_df[col].std()
                            av_df.loc[vid, col+'_sem'] = raw_df[col].std() / np.sqrt(rep)
    ...
    for rid in args.rid:
        ...
        with h5py.File(h5.fil, 'r') as dfil:
            ...
            for name in ftypes:
                ...
                columns = fdict[name]
                if args.loop:
                    av_df = average_loop_h5(data, name, columns, v_df, u_df, **vars(args))
                else:
                    av_df = average_h5(data, name, columns, **vars(args))
                if args.verbose:
